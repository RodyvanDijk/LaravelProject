...
test('admin can see the games delete page', function () {
    $admin = User::find(3);
    Laravel\be($admin)
        ->get(route('games.delete', ['game' => $this->game->id]))
        ->assertViewIs('admin.games.delete')
...
test('salesperson can see the games delete page', function () {
...
test('user can not see the games delete page', function () {
    $user = User::find(1);
    Laravel\be($user)
        ->get(route('games.delete', ['game' => $this->game->id]))
        ->assertStatus(403);
