    ...
    <div class="card mt-6">
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                ...
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{route('tasks.update', ['task'=> $task->id])}}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input
                               ...
                               name="name" value="{{old('name',$task->task)}}" type="text" required>
                        @error('name') <span class="text-xs text-red-600"> De task voldoet niet aan de voorwaarde</span> @enderror
                    </label>
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input" name="description"
                               value="{{old('description',$project->description)}}" type="text" required>
                    </label>
