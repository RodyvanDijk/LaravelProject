...
@section('content')
    ...
    <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-gray-700">
            ...
            <td class="px-4 py-3 text-sm"><a href="{{ route('games.show', ['game' =>$item->id]) }}">Details</a></td>
            <td class="px-4 py-3">
                <div class="flex items-center space-x-4 text-sm">
                    <a href="{{route('games.edit', ['game' => $item->id])}}" id="edit-{{$item->id}}"><button class="flex items-center
...
{{--            @can('delete product')--}}
                <td>
                    <div class="flex items-center space-x-4 text-sm">
                        <a href="{{ route('games.delete', ['game' => $item->id]) }}" id="delete-{{$item->id}}">
