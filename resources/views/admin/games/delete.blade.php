        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('games.destroy', ['game' => $game->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <label class="block text-sm">
                        ...
                        @error('game') border-red-600 focus:border-red-400 focus:shadow-outline-red @enderror"
                               name="game" value="{{old('game', $game->game)}}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                        ...
                        @error('description') border-red-600 focus:border-red-400 focus:shadow-outline-red @enderror"
                                  name="description" type="text" disabled>{{old('description', $game->description)}}</textarea>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Category</span>
                        <select
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-black ldeading-tight
                                focus:outline-none focus:shadow-outline" name="category_id" id="category_id" disabled>
                    ...
                    <label class="block text-sm">
                        ...
                        @error('price') border-red-600 focus:border-red-400 focus:shadow-outline-red @enderror"
                               name="price" value="{{old('price', $game->price)}}" type="text" disabled />
                    </label>
                    <button type="submit" class="mt-2 px-4 py-2 text-sm font-medium leading-5 text-black bg-gray transition-colors duration-150
